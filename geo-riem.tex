...
\end{proof}
%\begin{ejem}[El haz tangente de $S^1$]
%	Consideremos las siguientes cartas en $S^1$:
%	\begin{align*}
	%	\begin{aligned}
		%		h_1:U_1=S^1\backslash\{-1\}&\to\R\\
		%		z=e^{i\theta}&\mapsto \theta^1\in(-\pi,\pi)\\\\
		%		\begin{tikzpicture}
			%			\draw (0,0) circle (1);
		...
		%	\end{aligned}\qquad
	%	\begin{aligned}
		%		h_1:U_2=S^1\backslash\{1\}&\to\R\\
		%		z=e^{i\theta}&\mapsto \theta^2\in(0,2\pi)\\\\
		%		\begin{tikzpicture}
			%			\draw (0,0) circle (1);
			%			\draw[fill=white] (1,0) circle (0.05);
			%		\end{tikzpicture}
