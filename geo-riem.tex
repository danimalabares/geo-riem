...
\begin{document}
	...
	\end{prop}
	\begin{obs}
		Dada una transformación lineal entre espacios vectoriales $f:V\to W$, la transformación lineal 
		\begin{align*}
			f^*:\tau^k(W)&\to\tau^k(V)\\
	...
	En un espacio vectorial $V$ de dimensión finita definamos una relación de equivalencia entre bases $\{v_1,\ldots,v_n\}\sim\{w_1,\ldots,w_n\}$ cuando el determinante de la transformación lineal $T:V\to V$ tal que $T(v_j)=w_j$ para toda $j$ es positivo. Sólo existen dos clases de equivalencia. La es una elección de una de estas clases de equivalencia es una orientación.
	
	\begin{defn}[\cite{DoCarmo}\textbf{, 4.4}]
		Una variedad diferenciable $M$ es \textbf{orientable} si admite un atlas $A$ para el cual los cambios de coordenadas tienen jacobiana con determinante positivo. La elección de tal estructura diferenciable se llama \textbf{orientación}, y una \textbf{variedad orientada} es una pareja $(M,A)$. Dos orientaciones cuya unión también satisface la condición de jacobianas con determinante positivo en los cambios de cordanas \textbf{determinan la misma orientación}
	\end{defn}
	\begin{obs}\leavevmode
		\begin{itemize}
			\item Si $M$ es orientable y conexa entonces existen exactamente dos orientaciones distintas.
			\item Los difeomorfismos preservan orientación en cuanto a que si $M$ y $N$ son difeomorfas, basta que una sea orientable para que la otra lo sea.
			\item Si $M$ y $N$ son variedades difeomorfas orientadas, un difeomorfismo puede preservar o invertir la orientación.
	...
	Comparemos con la definición de \cite{Loring-dif} en su libro \textit{Differential Geometry: Connections, Curvature and Characterstic Classes}:
...
%	los elementos básicos del módulo $\T^r_s(M)$ serán los campos tensoriales que a cada punto de la variedad asignan el producto tensorial de un elemento de $\bigotimes_rTM^*$ y uno de $\bigotimes_sTM$
	...
	\end{lema}
	que es lo que esperaríamos de acuerdo a que
		\begin{teo}[\cite{Loring-dif}\textbf{, 18.8}]
				Si $v_1,\ldots,v_n$ es una base de $V$ y $w_1,\ldots,w_m$ es una base de $W$, entonces
				\[\{v_i\otimes w_j:1\leq i\leq n,1\leq j\leq n\}\]
				es una base de $V\otimes W$.
	...
	\chapter{Referencias}	\printbibliography[heading=none]
