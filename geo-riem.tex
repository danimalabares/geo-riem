...
\begin{document}
	...
	\begin{prop}[\cite{Lee-riem}\textbf{, 6.11}]
		Sea $(M,g)$ una variedad Riemanniana, $p\in M$ y $B$ una bola geodésica (=normal) alrededor de $p$. Salvo reparametrizaciones, la geodésica $\gamma:[0,c]\to B$ es la única curva que minimiza la distancia entre $\gamma(0)$ y $\gamma(c)$.
	...
	\begin{proof}
		...
		Notemos que el punto $\gamma(c)$ tiene coordenadas $cv$, así que $r(\gamma(c))=|rv|=c$. Además, la función $r\circ\sigma$ es continua en $[a_0,b_0]$ y suave (por pedazos si $\sigma$ es suave por pedazos). Esto nos permite calcular:
		\begin{align*}
			\ell(\gamma)&=c\\
			&=r(\gamma(c))\\
	...
	\begin{proof}[Solución]
		...
		Para usar que $X$ es ortogonal a los conjuntos de nivel tomamos una curva $\alpha:(-\delta,\delta)\to f^{-1}(c)$ cuya velocidad inicial $Y=\alpha'(0)$ es tangente al conjunto de nivel. Luego $f\circ\alpha\equiv c$, así que 
		...
		\[\nabla f=\lambda X\]
		sustituyendo en la ecuación anterior concluimos que $\lambda=1$.
