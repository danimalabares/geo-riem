...
%			&=-D_{\partial k}D_{\partial_\ell}\frac{\partial}{\partial x^j}+\sum_m\left(\frac{\partial}{\partial x^\ell}\Gamma_{kj}^m\right)\frac{\partial}{\partial x^m}+\sum_m\Gamma^m_{kj}D_{\partial_\ell}\frac{\partial}{\partial x^m}
	...
	Ahora veamos que no sólo el tensor de curvatura determina la curvatura seccional, sino al contrario también.
...
%	\end{proof}
	...
	\begin{ejem}
		Tomemos $M=\R^3$, $p=(0,0,0)$ y $g(u,v)=u_1v_1+u_2v_2-u_3v_3$. ¿Cómo es un plano degenerado $\Pi\subset T_pM$? ¿Qué tal el plano generado por $e_2$ y $e_3$?
	\end{ejem}
	
	Para ver cómo la curvatura seccional determina el tensor de curvatura, consideremos un plano degenerado $\Pi$ con base $u,v$ y $\Pi_n$ una sucesión de planos generador por $u_n,v_n$ que aproximan a $\Pi$ en el sentido de que $u_n\to u$ y $v_n\to v$. Ahora escojamos un vector de la siguiente manera:
	...
	Luego,
	\[Q(v,u+\delta x)=Q(v,u)+2\delta b+\delta^2 Q(v,x)\]
	para algún $\delta$ que nos ayude a aproximar $u$ como planteamos al principio.
